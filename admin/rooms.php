...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1 && isset($_GET['venue_id'])) {
    // Return just the specific venue card HTML
    $venue_id = (int)$_GET['venue_id'];
    // You would need to fetch and output just that card here
    // For simplicity, we'll let the regular page handle it for now
    exit;
}
...
<!-- Crop Modal -->
<div id="cropModal" style="display:none;position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,0.82);align-items:center;justify-content:center;">
    <div style="background:#fff;border-radius:16px;overflow:hidden;max-width:680px;width:calc(100% - 2rem);box-shadow:0 30px 80px rgba(0,0,0,0.5);display:flex;flex-direction:column;">
        <!-- Modal header -->
        <div style="background:linear-gradient(120deg,var(--red),var(--red-dark));padding:.9rem 1.3rem;display:flex;align-items:center;justify-content:space-between;">
            <div style="color:white;font-weight:700;font-size:.95rem;display:flex;align-items:center;gap:.5rem;">
                <i class="bi bi-crop"></i> Crop & Position Photo
            </div>
            <button onclick="closeCropModal()" style="background:rgba(255,255,255,.18);border:none;color:white;width:30px;height:30px;border-radius:50%;cursor:pointer;font-size:1rem;display:flex;align-items:center;justify-content:center;">
                <i class="bi bi-x"></i>
            </button>
        </div>
        <!-- Instructions bar -->
        <div style="background:#fff8f8;border-bottom:1px solid #f0d0d0;padding:.55rem 1.3rem;font-size:.75rem;color:#8b0000;display:flex;align-items:center;gap:.5rem;">
            <i class="bi bi-info-circle-fill"></i>
            Drag to reposition &nbsp;·&nbsp; Scroll / pinch to zoom &nbsp;·&nbsp; The highlighted area is what customers will see
        </div>
        <!-- Crop area -->
        <div style="padding:1rem;background:#2c2c2c;max-height:420px;overflow:hidden;display:flex;align-items:center;justify-content:center;">
            <img id="cropImg" src="" style="max-width:100%;max-height:390px;display:block;">
        </div>
        <!-- Aspect ratio + actions -->
        <div style="padding:.9rem 1.3rem;background:#f9f9f9;border-top:1px solid #eee;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.75rem;">
            <div style="display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;">
                <span style="font-size:.75rem;font-weight:600;color:#555;">Aspect ratio:</span>
                <button class="crop-ratio-btn active" data-ratio="16/9" onclick="setCropRatio(this,16/9)">16:9</button>
                <button class="crop-ratio-btn" data-ratio="4/3" onclick="setCropRatio(this,4/3)">4:3</button>
                <button class="crop-ratio-btn" data-ratio="1/1" onclick="setCropRatio(this,1)">1:1</button>
                <button class="crop-ratio-btn" data-ratio="free" onclick="setCropRatio(this,NaN)">Free</button>
            </div>
            <div style="display:flex;gap:.6rem;">
                <button onclick="closeCropModal()" style="background:#e5e7eb;border:none;padding:.5rem 1.1rem;border-radius:7px;font-size:.8rem;font-weight:600;cursor:pointer;color:#374151;">
                    Cancel
                </button>
                <button onclick="applyCrop()" style="background:var(--red);color:white;border:none;padding:.5rem 1.3rem;border-radius:7px;font-size:.8rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:.4rem;">
                    <i class="bi bi-check-lg"></i> Use this crop
                </button>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    // Bootstrap modals no longer needed for internal rooms (section removed)
});

/* ══════════════════════════════════════════════
   CROP MODAL — Cropper.js integration
   ══════════════════════════════════════════════ */
var _cropperInstance = null;
var _cropTargetId    = null;   // venue id
var _cropOrigFile    = null;   // original File object

function openCropModal(file, venueId) {
    _cropTargetId = venueId;
    _cropOrigFile = file;
    ...
    reader.onload = function(e) {
        ...
        // Small delay so image renders before Cropper measures it
        setTimeout(function() {
            ...
            // Highlight active ratio btn
            document.querySelectorAll('.crop-ratio-btn').forEach(function(b) {
                b.classList.toggle('active', b.dataset.ratio === '16/9');
            });
        }, 150);
    };
...
function closeCropModal() {
    ...
    const inp = document.getElementById('vfi_' + _cropTargetId);
    if (inp) inp.value = '';
    _cropTargetId = null;
    _cropOrigFile = null;
}

function setCropRatio(btn, ratio) {
...
/* ── Apply crop and upload via AJAX ── */
function applyCrop() {
    if (!_cropperInstance || !_cropTargetId) return;

    // Show loading indicator on the upload button
    const uploadBtn = document.getElementById('vub_' + _cropTargetId);
    ...
    // Determine mimeType FIRST before using it
    const origName = _cropOrigFile ? _cropOrigFile.name : 'cropped.jpg';
    const ext = origName.split('.').pop().toLowerCase();
    const mimeType = (ext === 'png') ? 'image/png' : (ext === 'gif' ? 'image/gif' : 'image/jpeg');

    canvas.toBlob(function(blob) {
        const croppedFile = new File([blob], origName, { type: mimeType, lastModified: Date.now() });

        // Get the is_primary checkbox value
        const isPrimaryCheckbox = document.querySelector('#vf_' + _cropTargetId + ' input[name="is_primary"]');
        const isPrimary = isPrimaryCheckbox ? (isPrimaryCheckbox.checked ? 1 : 0) : 0;

        // Create FormData for AJAX submission
        const formData = new FormData();
        formData.append('venue_action', 'upload_venue_image');
        formData.append('venue_id', _cropTargetId);
        formData.append('is_primary', isPrimary);
        formData.append('venue_image', croppedFile);
        ...
        .then(html => {
            // Parse the response to extract the message and updated card
            const parser = new DOMParser();
            const doc = parser.parseFromString(html, 'text/html');
            
            // Update message
            const newMessage = doc.querySelector('.alert');
            if (newMessage) {
                const messageContainer = document.querySelector('.rm-page > .alert')?.parentNode;
                if (messageContainer) {
                    const oldMessage = document.querySelector('.rm-page > .alert');
                    if (oldMessage) oldMessage.remove();
                    messageContainer.insertBefore(newMessage, messageContainer.firstChild);
                }
            }
            
            // Update the specific venue card
            const oldCard = document.querySelector('.vpc[data-venue-id="' + _cropTargetId + '"]');
            if (oldCard) {
                // Find the new card in the response
                const newCard = doc.querySelector('.vpc[data-venue-id="' + _cropTargetId + '"]');
                if (newCard) {
...
/* ── File picked: validate then open crop modal ── */
...
function uploadImageViaAjax(venueId, formData) {
    // Show uploading indicator
    const uploadBtn = document.getElementById('vub_' + venueId);
    ...
    .then(html => {
        ...
        // Refresh the specific card content without reloading the page
        refreshVenueCard(venueId);
        
        uploadBtn.innerHTML = originalText;
        uploadBtn.disabled = false;
        
        // Reset upload zone
        resetUploadZone(venueId);
    })
    .catch(error => {
        console.error('Upload error:', error);
        alert('Upload failed. Please try again.');
...
/* ── Refresh a specific venue card ── */
function refreshVenueCard(venueId) {
    fetch(window.location.href + '?venue_id=' + venueId + '&ajax=1')
    .then(response => response.text())
    .then(html => {
        const parser = new DOMParser();
        const doc = parser.parseFromString(html, 'text/html');
        const newCard = doc.querySelector('.vpc[data-venue-id="' + venueId + '"]');
        
        if (newCard) {
            const oldCard = document.querySelector('.vpc[data-venue-id="' + venueId + '"]');
            if (oldCard) {
                oldCard.outerHTML = newCard.outerHTML;
            }
        }
...
function resetUploadZone(venueId) {
    ...
    const nameEl = document.getElementById('vpn_' + venueId);
    const sizeEl = document.getElementById('vps_' + venueId);
    const zone = document.getElementById('vz_' + venueId);
