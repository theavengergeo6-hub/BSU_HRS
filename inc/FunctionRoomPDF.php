...
$putMulti($xOffice, $y1, $office, $rightColWOffice, 8.0, 3.8);
        ...
        $qty = function($v): int {
            if ($v === null) return 0;
            if (is_array($v)) return (int)($v['quantity'] ?? 0);
            return (int)$v;
        };
        ...
        if ($othersText !== '—' && $othersText !== '') {
            $put(112.0, 220.0, $othersText, 8.5, 80.0);
        }
    ...
    private function drawForm(\TCPDF $pdf): void {
        ...
        $this->sigPanelFixed($pdf, $lm + $panelW + 1, $y + 2, $panelW - 2,
            'Request received by:',
        ...
        // Panel 2 — Approved by
        $this->sigPanelFixed($pdf, $lm + $panelW * 2 + 1, $y + 2, $panelW - 2,
            'Approved by:',
    ...
    private function miscQtyText(array $misc, string $key): string {
        $v = $misc[$key] ?? null;
        if ($v === null) return '—';
        if (is_array($v)) $qty = (int)($v['quantity'] ?? 0);
        else $qty = (int)$v;
    ...
    private function miscOthers(array $misc): string {
        $out   = [];
        $known = [
            'basic_sound_system', 'round_table', 'banquet_chairs', 'view_board',
            'rectangular_table',  'mono_block_chairs', '_price_breakdown',
            '_estimated_total',   '_client_type', '_terms_agreed_by',
            '_terms_position',    '_terms_date',
        ];
        foreach ($misc as $k => $v) {
